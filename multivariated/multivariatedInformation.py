...
def run_extraction_process_multivariate(df_list, fileout, dx):
    new_df = None
    new_df_sz = 0
    time_hc = []
    time_fs = []
    ...
    for window_df in df_list:
        ...
        scaler = MinMaxScaler()
        window_norm = scaler.fit_transform(window_df.values)


        norms = np.linalg.norm(window_norm, axis=1)
        ...
        # salva último valor de cada variável
    ...
    time_dict = {'time_hc': time_hc, 'time_fs': time_fs}
...
class ExtractInformation:
    ...
    def run(self):
        ...
        for i, thread_df in enumerate(sublist_split):
            ...
            p = multiprocessing.Process(target=run_extraction_process_multivariate, args=(thread_df, filename, self.__d))
            p.start()
