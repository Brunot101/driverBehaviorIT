...
def run_extraction_process(df_list, fileout, dx):
    new_df = None
    new_df_sz = 0
    time_hc = []
    time_fs = []
    ...
    for window_df in df_list:
        ...
        if len(arr) < dx:
            ...
            t0 = time.time()
            data_probs = ordpy.ordinal_distribution(arr, dx=dx, dy=dy, return_missing=True)[1]
            tf = time.time()
            time_probs = tf - t0

            t0 = time.time()
            h, c = ordpy.complexity_entropy(data=data_probs, dx=dx, dy=dy, probs=True)
            tf = time.time()
            time_hc.append(tf - t0 + time_probs)

            
            t0 = time.time()
            f, s = ordpy.fisher_shannon(data=data_probs, dx=dx, dy=dy, probs=True)
            tf = time.time()
            time_fs.append(tf - t0 + time_probs)

        
        for col in window_df.columns:
            row[col] = window_df[col].values[-1]

        row["entropy_joint"] = h
        row["complexity_joint"] = c
        row["fisher_joint"] = f
...
class ExtractInformation:
    ...
    def run(self):
        ...
        for i, thread_df in enumerate(sublist_split):
            ...
            p = multiprocessing.Process(target=run_extraction_process, args=(thread_df, filename, self.__d))
            p.start()
